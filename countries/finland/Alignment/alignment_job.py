...
def main():
    ...
    base_dir = "/itet-stor/spfisterer/net_scratch/Downloading/countries/finland"
    csv_path = f"{base_dir}/links/finland_links.csv"
    ...
    downloaded_video_ids = glob.glob(os.path.join(base_dir, "downloaded_audio", "m3u8_streams", "*.opus"))
    downloaded_video_ids = [os.path.splitext(os.path.basename(video_id))[0] for video_id in downloaded_video_ids]
    print(f"Found {len(downloaded_video_ids)} downloaded video IDs")
    # Load all video IDs
    df = pd.read_csv(csv_path)
    all_video_ids = df["video_id"].astype(str).tolist()
    
    # Filter out already processed video IDs, only process videos that are downloaded, and only videos that have 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024 in the video_id
    video_ids_to_process = [vid for vid in all_video_ids if vid not in processed_video_ids and vid in downloaded_video_ids and "2017" in vid or "2018" in vid or "2019" in vid or "2020" in vid or "2021" in vid or "2022" in vid or "2023" in vid or "2024" in vid]
