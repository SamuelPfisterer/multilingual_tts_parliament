...
def download_and_process_generic_video(url: str, output_filename: str) -> bool:
    ...
    try:
        ...
        if os.path.exists(temp_dir) and not os.listdir(temp_dir):
            #os.rmdir(temp_dir)
            print(f"Skipping temp directory deletion to avoid race conditons")
...
def download_and_process_txt(txt_link: str, output_filename: str, redownload_transcripts: bool = True) -> bool:
    """
    Download text files using curl_cffi to bypass protections.
    
    Args:
        txt_link: Direct link to text file or endpoint that returns text
        output_filename: Desired output filename (without extension)
        redownload_transcripts: Whether to redownload even if exists
    """
    try:
        ...
        # Setup temporary and final paths
        temp_txt = os.path.join(temp_dir, f'{os.path.basename(output_filename)}.txt')
        final_txt = f'{output_filename}.txt'
        
        # Check if file already exists
        if os.path.exists(final_txt) and not redownload_transcripts:
            logging.info(f"Text file {final_txt} already exists and redownload not requested")
            return True

        # Set headers to look more like a real browser
        headers = {
            "Referer": txt_link.split('/')[0] if '/' in txt_link else txt_link,
            "Accept": "text/plain,*/*",
            "Accept-Language": "en-US,en;q=0.9",
            "Connection": "keep-alive",
        }
        ...
        for browser in browser_types:
            try:
                logging.info(f"Trying to download {txt_link} with {browser} impersonation...")
        ...
        if not download_success:
            logging.error(f"All browser impersonations failed for {txt_link}")
            return False
            
        # Move to final location
        shutil.move(temp_txt, final_txt)
        logging.info(f"Successfully processed: {txt_link}")
        ...
        logging.error(f"Error processing {txt_link}: {str(e)}")
        return False
    finally:
        # Cleanup temp directory if empty
        if os.path.exists(temp_dir) and not os.listdir(temp_dir):
