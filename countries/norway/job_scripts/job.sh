...
#SBATCH --output=/itet-stor/spfisterer/net_scratch/Downloading/countries/norway/logs/%j.out
#SBATCH --error=/itet-stor/spfisterer/net_scratch/Downloading/countries/norway/logs/%j.err
#SBATCH --mem=8G
#SBATCH --nodes=1
#SBATCH --cpus-per-task=2
#SBATCH --array=0-3  # 4 jobs to handle 1904 sessions
#SBATCH --job-name=norway_full

ETH_USERNAME=spfisterer
PROJECT_NAME=Downloading/countries/norway
...
TOTAL_FILES=1904  # Total number of sessions (minus header)
FILES_PER_JOB=476  # Split into 4 jobs (~476 files each)
