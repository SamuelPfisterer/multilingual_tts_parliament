...
def main():
    ...
    base_dir = "/itet-stor/spfisterer/net_scratch/Downloading/countries/iceland"
    csv_path = f"{base_dir}/links/final_iceland_links_to_process.csv"
    ...
    # Get all the downloaded transcripts 
    transcript_dir = f"{base_dir}/downloaded_transcript/processed_text_transcripts"
    transcript_files = glob.glob(os.path.join(transcript_dir, "*.txt"))
    transcript_ids = [os.path.basename(file).replace(".txt", "") for file in transcript_files]
    ...
    video_ids_to_process = [vid for vid in all_video_ids if vid not in processed_video_ids and vid in transcript_ids]
