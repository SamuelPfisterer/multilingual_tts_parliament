...
def main():
    ...
    base_dir = "/itet-stor/spfisterer/net_scratch/Downloading/countries/lithuania"
    csv_path = f"{base_dir}/links/lithuania_links.csv"
    ...
    processed_files = glob.glob(os.path.join(alignment_output_dir, "*_alignment_summary.json"))
    processed_video_ids = set()
    for file_path in processed_files:
        file_name = os.path.basename(file_path)
        # Extract the video ID from filenames like "slovenia_1_Izredna_0_18052022_alignment_summary.json"
        if "_alignment_summary.json" in file_name:
            video_id = file_name.replace("_alignment_summary.json", "")
    ...
    # Filter out downloaded videos. Downloaded videos are in downloaded/audio/mp3_audio as .opus files, and the video_id is the filename without the .opus extension
    downloaded_video_ids = [os.path.basename(file).replace(".opus", "") for file in glob.glob(f"{base_dir}/downloaded/audio/mp3_audio/*.opus")]
    
    # Filter out already processed video IDs
    video_ids_to_process = [vid for vid in all_video_ids if vid not in processed_video_ids and vid not in downloaded_video_ids]
